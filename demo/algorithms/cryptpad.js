		...
		xhr.onload = function () {
			...
			if (arrayBuffer) {
				...
				Cryptopad.decrypt(u8, cryptoKey, function (err, decrypted) {
					...
					/**
					 * Works because the library is scoped in the index but in
					 * static media-tag library case the script is added inside the
					 * inner and window.document.dispatchEvent never hits decryptionEvent handler.
					 * 
					 * mediaObject.return(); only works but to handle metadata outside the inner
					 * media-tag must send events to his tag which may be observed by the index.
					 */
